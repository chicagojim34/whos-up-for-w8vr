...
export function generateIcsCalendar(events: EventItem[], calendarTitle = "W8VR Schedule"): string {
  ...
  for (const evt of events) {
    // Event start time is the host meetup time or primary event time
    const startTimeStr = evt.meetupTime || evt.time || '7:00 PM';
    const startDate = parseEventDateTime(evt.date, startTimeStr);
    ...
    if (evt.showtime && evt.meetupTime) {
      ...
      descParts.push(`🕒 START TIME: ${evt.time}`);
    }
