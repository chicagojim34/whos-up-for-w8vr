...
export const CalendarSubscribeModal: React.FC<CalendarSubscribeModalProps> = ({
  ...
  const filteredEvents = events.filter(e => {
    if (feedFilter === 'attending') return e.status === 'Attending';
      ...
      <div className="flex flex-col gap-5 text-text-dark">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 bg-surface-low p-1 rounded-2xl">
            ...
            >
              Confirmed Events Only ({events.filter(e => e.status === 'Attending').length})
            </button>
            <button
              type="button"
            ...
            >
              All Scheduled + Waitlist ({events.filter(e => e.status === 'Attending' || e.status === 'Pending RSVP' || e.status === 'Waitlisted').length})
            </button>
          </div>
        </div>
